...
export default function DtrpCtrTable() {
  // ------------ States -----------------
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/dtrpctr`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: dtrpCtrType) =>
      ...
      <Table data={dataTable} theme={theme} pagination={pagination}>
        {(tableList: []) => (
          <>
            ...
            <Body>
              {tableList.map((item: dtrpCtrType) => (
