...
export default function DtrpCtrToOvercomeTable() {
  // ------------ States -----------------
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/dtrpctrToOvercome`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: DtrpCtrToOvercomeType) =>
      ...
      <Table data={dataTable} theme={theme} pagination={pagination}>
        {(tableList: []) => (
          <>
            ...
            <Body>
              {tableList.map((item: DtrpCtrToOvercomeType) => (
