...
export default function ExtinguisherTable() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/extinguisher`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: ExtinguisherType) =>
      ...
      <Table data={dataTable} theme={theme} pagination={pagination}>
        {(tableList: []) => (
          <>
            ...
            <Body>
              {tableList.map((item: ExtinguisherType) => (
                <Row key={item.id} item={item}>
                  <Cell>{item.company}</Cell>
                  <Cell>{moment(item.exchangeDate).format("L")}</Cell>
