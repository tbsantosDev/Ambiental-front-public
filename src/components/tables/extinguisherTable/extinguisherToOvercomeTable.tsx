...
type ExtinguisherToOvercomeType = {
  id: number;
  company: string;
...
export default function ExtinguisherToOvercomeTable() {
  // ------------ States -----------------
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/extinguisherToOvercome`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: ExtinguisherToOvercomeType) =>
          ...
          <>
            <Header>
              <HeaderRow>
                ...
                <HeaderCell resize={resize}>Recarga</HeaderCell>
            ...
            <Body>
              {tableList.map((item: ExtinguisherToOvercomeType) => (
                <Row key={item.id} item={item}>
                  <Cell>{item.company}</Cell>
                  <Cell>{moment(item.exchange_date).format("L")}</Cell>
