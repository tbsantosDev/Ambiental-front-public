...
export default function BrigadeToOvercomeTable() {
  // ------------ States -----------------
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/brigadeToOvercome`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: BrigadeToOvercomeType) =>
      ...
      <Table data={dataTable} theme={theme}  pagination={pagination}>
        {(tableList: []) => (
          <>
            ...
            <Body>
              {tableList.map((item: BrigadeToOvercomeType) => (
