...
type CertificateToOvercomeType = {
  id: number;
  company: string;
  term: number;
  due_date: string;
...
export default function CertificateToOvercomeTable() {
  // ------------ States -----------------
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/certificateToOvercome`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: CertificateToOvercomeType) =>
      ...
      <Table data={dataTable} theme={theme} pagination={pagination}>
        {(tableList: []) => (
          <>
            ...
            <Body>
              {tableList.map((item: CertificateToOvercomeType) => (
