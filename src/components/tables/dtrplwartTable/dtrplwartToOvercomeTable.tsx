...
export default function DtrpLwartToOvercomeTable() {
  // ------------ States -----------------
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/dtrplwartToOvercome`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: DtrpLwartToOvercomeType) =>
      ...
      <Table data={dataTable} theme={theme} pagination={pagination}>
        {(tableList: []) => (
          <>
            ...
            <Body>
              {tableList.map((item: DtrpLwartToOvercomeType) => (
