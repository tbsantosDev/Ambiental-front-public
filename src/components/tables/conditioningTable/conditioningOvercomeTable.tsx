...
type ConditioningToOvercomeType = {
  id: number;
  company: string;
  description: string
  term: number;
...
export default function ConditioningToOvercomeTable() {
  // ------------ States -----------------
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/conditioningsToOvercome`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: ConditioningToOvercomeType) =>
      ...
      <Table data={dataTable} theme={theme} pagination={pagination}>
        {(tableList: []) => (
          <>
            ...
            <Body>
              {tableList.map((item: ConditioningToOvercomeType) => (
