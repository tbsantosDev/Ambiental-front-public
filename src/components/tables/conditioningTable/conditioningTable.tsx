...
export default function ConditioningTable() {
  // ------------ States -----------------
  ...
  const fetchData = useCallback(async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_BASEURL}/conditionings`;
  ...
  const dataTable = {
    nodes: data.nodes.filter((item: ConditioningType) =>
      ...
      <Table data={dataTable} theme={theme} pagination={pagination}>
        {(tableList: []) => (
          <>
            ...
            <Body>
              {tableList.map((item: ConditioningType) => (
